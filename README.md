...
1. プリセットの選択（デフォルト: `default`）
2. 対象リポジトリ（`owner/repo`）の入力
3. リポジトリの存在確認（public / private を表示）
4. Dependabot alerts + security updates を有効化するかの確認（デフォルト: はい）
5. 選択したプリセットの任意 rulesets の有効化確認（リポジトリ単位）
6. `--dry-run` の実行（任意・デフォルト: はい）
7. 本番適用の実行（任意・デフォルト: いいえ）
...
# プリセットを指定して適用（未指定時は default）
./apply.sh owner/repo --preset github-flow
```

## プリセット

運用パターンごとに settings/actions/rulesets を切り替えられるよう、`settings/presets/<name>/` 配下にプリセットとしてまとめている。各プリセットは `settings.json` / `actions.json` / `actions-selected.json` / `rulesets/*.json` をフルセットで持つ独立ディレクトリで、ベースとの差分マージは行わない。

`--preset <name>` で選択する（未指定時は `default`）。`setup.sh` では `settings/presets/*/` を走査した一覧から対話で選ぶ。

現在用意されているプリセット:

| プリセット    | 想定運用                                                                                                                     |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `default`     | ブランチ運用・マージ方式を問わない汎用設定                                                                                     |
| `github-flow` | 単一 `main` + Conventional Commits + [cocogitto](https://github.com/cocogitto/cocogitto) による自動リリースを前提とした運用 |

`github-flow` プリセットは、対象リポジトリに以下の workflow が配置されている前提で rulesets を定義する（workflow ファイル自体の配置は[スコープ外](#スコープ外)）。

- `.github/workflows/no-fixup-commits.yml`（`no-fixup-commits` ステータスチェック）
- `.github/workflows/lint-commits.yml`（`commitlint` ステータスチェック）
- `.github/workflows/release.yml`（`release` ステータスチェック）

新しいプリセットを追加する場合は、`settings/presets/<name>/` に上記4種のファイルを揃える。

## 適用される設定（8 ステップ）

| #   | 項目                                                                       | 設定ファイル / API                                                                |
| --- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| 1   | 一般設定（Issues、マージ方法、Secret scanning など）                       | `settings/presets/<preset>/settings.json`                                          |
| 2   | Release immutability の有効化                                              | `PUT .../immutable-releases`                                                        |
| 3   | Actions 権限                                                               | `settings/presets/<preset>/actions.json`                                           |
| 4   | 許可する Actions（`allowed_actions: selected` の場合）                     | `settings/presets/<preset>/actions-selected.json`                                  |
...
| 8   | Rulesets                                                                   | `settings/presets/<preset>/rulesets/*.json`（常時適用 + `--with-rulesets` で任意） |

`<preset>` は `--preset` で指定したプリセット名（未指定時は `default`）。

### 現在の推奨値の概要（`default` プリセット）

**一般設定** (`settings/presets/default/settings.json`)
...
**Actions** (`settings/presets/default/actions.json`, `settings/presets/default/actions-selected.json`)
...
- 常時適用: `settings/presets/default/rulesets/default-branch-protection.json`
...
- 任意適用: `settings/presets/default/rulesets/*.json` のうち `REQUIRED_RULESETS` に含まれないファイル（`--with-rulesets <basename>`）
...
- 常時適用は `rulesets-common.sh` の `REQUIRED_RULESETS`（basename のリストで、全プリセット共通）、それ以外は `--with-rulesets <basename>`
- `REQUIRED_RULESETS` 自体はプリセット間で共通のリストのため、プリセットごとに常時適用するステータスチェックを変えたい場合は、そのプリセット自身の `default-branch-protection.json` の中身（`rules` 配列）を変更する。`github-flow` プリセットでは `required_status_checks` ルールを追加し、`no-fixup-commits` / `commitlint` / `release` を常時必須のステータスチェックとしている（詳細は `settings/presets/github-flow/rulesets/default-branch-protection.json` を参照）

各プリセット配下の JSON の値を編集すれば、適用内容をリポジトリやチームの方針に合わせて変更できる。

## dry-run の挙動

`--dry-run` では API への書き込みは行わない。

- **JSON 設定ステップ（1, 3, 4）**: ライブ API の現在値と `settings/presets/<preset>/` の desired 値を比較し、差分をカラー diff で表示する。API が返さないフィールドは desired 値で補完して比較する。
...
| プリセットが前提とする workflow ファイルの配置（例: `lint-commits.yml`、`no-fixup-commits.yml`、`release.yml`） | 本ツールは settings/actions/rulesets の API 適用のみ行う。workflow ファイル自体は対象リポジトリ側で別途配置する |
...
├── rulesets-common.sh    # プリセット解決・常時適用 ruleset 定義（apply.sh / setup.sh 共有）
└── settings/
    └── presets/
        ├── default/
        │   ├── settings.json                  # 一般設定
        ...
        └── github-flow/
            ├── settings.json
            ├── actions.json
            ├── actions-selected.json
            └── rulesets/
                └── default-branch-protection.json  # no-fixup-commits / commitlint / release を常時必須ステータスチェックとして含む
```
